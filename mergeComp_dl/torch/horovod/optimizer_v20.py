...
from horovod.torch.mpi_ops import allreduce_async_, allreduce
...
class _DistributedOptimizer(torch.optim.Optimizer):
    def __init__(self, params, named_parameters, compression, compression_start_iter=1, threshold=2**20,
        ...
        self._iterations = 0
        self._compression_start_iteration = compression_start_iter
        self._tensor_thres = threshold
    ...
    def _compression_iteration(self):
        if self._iterations < self._compression_start_iteration:
            return False
        else:
            return True
    ...
    def _communicate_grad_async(self, p):
        ...
        #     print("tensor name=", name, tensor.numel(), hvd.rank(), flush=True)

        if self._compression_iteration() and tensor.numel() > self._tensor_thres:
            handle, ctx = self._compression.send_step(tensor, name)
        else:
            if self.op == Average:
                # Split average operation across pre/postscale factors
                # C++ backend will apply additional 1 / size() factor to postscale_factor for op == Average.
    ...
    def synchronize(self):
        ...
        for p, (handles, ctx) in self._handles.items():
            if isinstance(p, tuple):
                # This was a grouped result, need to unpack
                outputs = synchronize(handles)
                ...
                if self._compression_iteration() and p.grad.numel() > self._tensor_thres:
                    tensor = self._compression.receive_step(handles, ctx)
                    # print("[compression synchronize] name:", self._parameter_names.get(p), "size:", tensor.numel(), "handles:", handles, flush=True)
                    p.grad.set_(tensor)
                else:
                    tensor = synchronize(handles)
                    p.grad.set_(tensor)
        self._handles.clear()
...
def DistributedOptimizer(optimizer, named_parameters=None,
    ...
    Arguments:
        ...
        compression_start_iter: The start iteration for gradient compression to skip the warmup phase.
    ...
    if op != Adasum or size() == 1:
        ...
        return cls(optimizer.param_groups, named_parameters, compression, compression_start_iter, threshold, backward_passes_per_step, op,
