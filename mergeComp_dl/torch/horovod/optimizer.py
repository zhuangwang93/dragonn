            ...
            if isinstance(p, tuple):
                ...
                if p.grad.is_sparse:
                    ...
                    if self._compression_iteration() and p.grad.numel() > self._tensor_thres:
                        tensor = self._compression.receive_step(handle, ctx)
                        # print("[compression synchronize] name:", self._parameter_names.get(p), "size:", tensor.numel(), "handle:", handle, flush=True)
                        p.grad.set_(tensor)
                    else:
                        # When handle is a callable function, it returns the aggregated tensor result
                        output = synchronize(handle) if not callable(handle) else handle()
